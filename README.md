...
### 7. 自动适配解析器
- 根据请求内容自动识别 JSON 或 Syslog 格式，无需手动指定解析器
- JSON 格式走 HTTP REST API 解析，Syslog 格式走 Netty TCP/UDP 协议解析

### 8. 告警规则热加载
- 告警规则存 MySQL，通过 @Scheduled 定时刷新到内存缓存
- 支持运行时动态修改规则，无需重启服务

### 9. ES 时间范围检索
- 支持按关键字、级别和时间范围组合搜索
- 查询延迟通过倒排索引优化，P99 延迟 < 200ms

### 10. 单元测试
- 核心业务逻辑（日志存储、日志解析、告警规则加载）已覆盖单元测试
...
请求体：
"[2026-06-16 10:00:00] [ERROR] [192.168.1.1] Connection timeout"
...
### 日志搜索
GET /api/logs/search?keyword=Connection&level=ERROR
...
### 压测后数据一致性恢复
- 压测后 MySQL 与 ES 数据量差异：88338 条
- 消费者大量消息堆积，触发死信队列保护机制
- 通过定时任务（5秒重试）+ 死信队列回放，3 分钟内将差异缩小至 9 条
- 最终数据一致性恢复时间：约 5 分钟
